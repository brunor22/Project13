...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* Top half: table */
  .table-section { flex: 0 0 50%; display: flex; flex-direction: column; overflow: hidden; border-bottom: 2px solid var(--border); }
  /* Bottom half: widgets side by side */
  .widgets-section { flex: 0 0 50%; display: flex; overflow: hidden; }
  .tv-widgets-header { padding: 6px 12px; border-bottom: 1px solid var(--border); font-size: 11px; font-weight: 600; color: var(--muted); display: flex; align-items: center; gap: 8px; flex-shrink: 0; background: var(--surface); }
  .tv-col-ticker { color: var(--accent); font-weight: 700; }
  .tv-profile-embed { flex: 1; display: flex; flex-direction: column; border-right: 1px solid var(--border); overflow: hidden; background: #fff; }
  .tv-chart-embed  { flex: 1; display: flex; flex-direction: column; overflow: hidden; background: #fff; }
  .tv-embed-label { padding: 5px 10px; font-size: 10px; text-transform: uppercase; letter-spacing: 0.6px; color: var(--muted); background: var(--surface2); border-bottom: 1px solid var(--border); flex-shrink: 0; }
  .tv-embed-body  { flex: 1; overflow: hidden; }
  ...
  .v-pass       { background: rgba(153,27,27,0.08);  color: var(--red); border: 1px solid rgba(153,27,27,0.25); }

  .eps-beat-y { color: var(--green); font-weight: 600; }
  .eps-beat-n { color: var(--red); }

  /* ── Detail panel ── */
  .detail-panel { display: none; position: fixed; right: 0; top: 0; width: 420px; height: 100vh; background: var(--surface); border-left: 1px solid var(--border); overflow: hidden; z-index: 100; box-shadow: -4px 0 24px rgba(0,0,0,0.5); flex-direction: column; }
  ...
  .tv-btn-row { display: flex; gap: 8px; margin-top: 10px; }
  .tv-btn { flex: 1; display: flex; align-items: center; justify-content: center; gap: 6px; padding: 7px 10px; border-radius: 6px; border: 1px solid var(--border); background: var(--surface2); color: var(--text); font-size: 12px; font-weight: 600; cursor: pointer; transition: all 0.15s; text-decoration: none; }
...
<div class="layout">
  ...
  <div class="main">
    ...
    <!-- Bottom 50%: profile (left) | chart (right) -->
    <div class="widgets-section">
      <div class="tv-profile-embed" id="tv-profile-embed">
        <div class="tv-embed-label">Company Profile &nbsp;<span class="tv-col-ticker" id="tv-col-ticker">— select a stock —</span></div>
        <div class="tv-embed-body" id="tv-profile-body">
          <div class="tv-placeholder">Click a row to load profile</div>
        </div>
      </div>
      <div class="tv-chart-embed" id="tv-chart-embed">
        <div class="tv-embed-label">Chart</div>
        <div class="tv-embed-body" id="tv-chart-body">
...
function showDetail(ticker) {
  ...
  document.getElementById("detail-header-content").innerHTML = `
    ...
    <div style="margin:6px 0">${fmtVerdict(s.verdict)} &nbsp; ${fmtComposite(s.composite)}</div>
    <div class="tv-btn-row">
      <button class="tv-btn" onclick="openChartWindow('${s.ticker}')">&#9661; Chart</button>
      <button class="tv-btn" onclick="openProfileWindow('${s.ticker}')">&#9432; Profile</button>
    </div>
  `;
  ...
  const fundsDiv = document.createElement("div");
  fundsDiv.innerHTML = `
...
function loadEmbeddedWidgets(ticker) {
  ...
  // Chart
  const chartEl = document.getElementById("tv-chart-body");
  chartEl.innerHTML = "";
  ...
  // Profile
  const profEl = document.getElementById("tv-profile-body");
  profEl.innerHTML = "";
...
<div class="tradingview-widget-container" style="height:100%;width:100%">
  ...
  <div class="tradingview-widget-copyright" style="font-size:11px;padding:4px 8px;color:#666">
  ...
  window.open(URL.createObjectURL(blob), `tv_chart_${ticker}`, "width=1100,height=650,resizable=yes");
}
...
<div class="tradingview-widget-container" style="height:100%;width:100%">
  ...
  const blob = new Blob([html], {type:"text/html"});
  window.open(URL.createObjectURL(blob), `tv_profile_${ticker}`, "width=460,height=600,resizable=yes");
}
