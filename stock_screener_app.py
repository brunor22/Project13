...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* profile widget */
  .detail-section .tradingview-widget-container { width: 100%; }
  .detail-section .tradingview-widget-container__widget { min-height: 400px; }
  .detail-section .tradingview-widget-copyright { font-size: 11px; color: var(--muted); padding: 4px 0; }
...
function showDetail(ticker) {
  ...
  // Company profile widget — injected via DOM so script executes
  const detailBody = document.getElementById("detail-content");
  detailBody.innerHTML = "";

  const profileWrap = document.createElement("div");
  profileWrap.className = "detail-section";
  profileWrap.innerHTML = `<h3>Company Profile</h3>`;
  const profileContainer = document.createElement("div");
  profileContainer.className = "tradingview-widget-container";
  const profileWidget = document.createElement("div");
  profileWidget.className = "tradingview-widget-container__widget";
  const profileCopyright = document.createElement("div");
  profileCopyright.className = "tradingview-widget-copyright";
  profileCopyright.innerHTML = `<a href="https://www.tradingview.com/symbols/${encodeURIComponent(s.ticker)}/" rel="noopener nofollow" target="_blank"><span class="blue-text">${s.ticker} key facts</span></a><span> by TradingView</span>`;
  ...
  profileContainer.appendChild(profileWidget);
  profileContainer.appendChild(profileCopyright);
  profileContainer.appendChild(profileScript);
  profileWrap.appendChild(profileContainer);
  detailBody.appendChild(profileWrap);

  // Remaining fundamentals via innerHTML
